...
function tegs_case(dsratio, csratio, en_cap, cap, ch_cap, outputs_path, mysetup, myinputs, OPTIMIZER)
    ...
    @expression(EP, SOC_interior[t in INTERIOR_SUBPERIODS], EP[:vS][end,t-1]/20330)

    @constraint(EP, TEGSeffcharge_interior[t in INTERIOR_SUBPERIODS], EP[:vCHARGE][end,t] <= 
    (sin(EP[:SOC_interior][t])))
