...
h=10; %linspace(1,100,100); %hours of storage
p=100*1e6; %Power output in W
...
% d=1;
ff_dis=1; %Discharge flowrate factor
...
Cost_Heater_elements=SF*18*11500*(p_t_charge_nom/200e6);      %Element cost (cost ~$18/m graphite store)
Cost_Heater_controller=SF*(p_t_charge_nom*5/1000);            %Controller Cost $5/kW thyristor controllers
Cost_Heater_pipes=SF*(103*10*555)*(p_t_charge_nom/200e6);     %C Pipe cost from graphite store
Cost_Heater_headers=SF*3*3*2700*(p_t_charge_nom/200e6);       %quote for custom headers
Cost_Heater_Cfelt=SF*2.4*Cfelt*(p_t_charge_nom/80e6);         %Graphite felt insulation cost
Cost_Heater_ASO=SF*2.6*ASO*(p_t_charge_nom/80e6);             %Al2O3-SiO2 insulation cost
Cost_Heater_FG=SF*1*FG*(p_t_charge_nom/80e6);                 %FG insulation cost
...
CPP=(Cost_MPV+Cost_Heater+Cost_Pumps_pipes_dis+Cost_Pumps_pipes_ch+Cost_Controls+Cost_CPP_construction)./p; %$/W
CPES = CPE+CPP./h.*1000;
...
CPE_Array = [CPE_TankMaterial CPE_Medium (CPE_GraphiteInsulation+CPE_AluminaInsulation+CPE_FiberglassInsulation)...
    CPE_InertContainment CPE_TankBase CPE_CoolingBase CPE_Construction]*0.5*2;
...
CPP_PumpsPiping= Cost_Pumps_pipes_dis./p;
