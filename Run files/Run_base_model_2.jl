...
function tegs_case(dsratio, csratio, outputs_path, mysetup, myinputs, OPTIMIZER)
    ...
    @constraint(EP, TEGSeffpower_interior[t in INTERIOR_SUBPERIODS], EP[:vP][end,t] * dsratio <= EP[:vS][end,t-1])
    @constraint(EP, TEGSeffpower_start[t in START_SUBPERIODS], EP[:vP][end,t] * dsratio <= EP[:vS][end,t+hours_per_subperiod-1])

    ## Constrain charging power based on state of charge. Reaches rated capacity at 0% state of charge
    @constraint(EP, TEGSeffcharge_interior[t in INTERIOR_SUBPERIODS], EP[:vCHARGE][end,t] * csratio <= EP[:eTotalCapCharge][end] * csratio - EP[:vS][end,t-1])
    @constraint(EP, TEGSeffcharge_start[t in START_SUBPERIODS], EP[:vCHARGE][end,t] * csratio <= EP[:eTotalCapCharge][end] * csratio - EP[:vS][end,t+hours_per_subperiod-1])
